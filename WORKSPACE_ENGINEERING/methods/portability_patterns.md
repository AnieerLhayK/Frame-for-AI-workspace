...
The change surface planner (`scripts/workspace/plan_change_surface.py`) must handle three absolute path formats on one host:
...
- `scripts/workspace/bootstrap_workspace.py` (bounded discovery, no drive scans)
- `scripts/validation/validate_manifest.py` (structural checks)
- `scripts/workspace/migration_dry_run.py` (what changes before a move)
- `shared/workspace/manifest_portability_policy.md` (rules for safe migration)
