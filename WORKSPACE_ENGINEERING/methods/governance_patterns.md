...
### Dependency Contracts Belong In One Registry

For a multi-skill package, keep each skill's runtime protocol dependency list
in the workspace manifest and make package validation read that list. A local
`SHARED_PROTOCOLS.md` should be a compact pointer to the canonical declaration,
not a second hand-maintained list of paths. This preserves a readable local
entry point while eliminating drift across skill docs, package manifests, and
runtime routing.

Classify dependencies by the active responsibility, not by the package that
contains them. A runtime `text_only` character should route diagnosis and
maintenance work outward rather than loading record, patch, versioning, and
reporting policy on every writing request. Conversely, diagnosis and
maintenance skills keep the record protocol required because it constrains
their durable effects.

Task tests normally belong in validation, not the resolver's initial required
context. Make a test required only when its source is itself the object being
designed; otherwise load it after the implementation approach is selected or a
validation failure needs it. Measure the resolver output after every routing
change so a lower token budget does not hide a missing safety contract.
...
The change surface planner (`scripts/workspace/plan_change_surface.py`) compares named alternative file sets against resolved task authority before any edit:
