...
- Never switch branches or rewrite history in the shared workspace in place; run `checkout`, `switch`, `reset`, and `rebase` work in an isolated `git worktree`.
- Before integration, run `git diff --name-status <target>...<source>` and stop for explicit user approval if it deletes or replaces content under `skills/`, `external-skills/`, `packages/`, `shared/`, or `PROJECT_CONTEXT/`.
