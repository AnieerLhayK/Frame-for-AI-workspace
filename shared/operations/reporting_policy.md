...
`reports/agent-experiments/<agent-id>/` is another narrow exception. It accepts
only bounded low-risk records from a validated testing registration, inside
that agent's exact subdirectory. The directory is created on demand only when
a real experiment begins. Experiment records are evidence, not activation or
authority.

`reports/agent-requests/` and `reports/agent-experiments/<agent-id>/` are
controlled recording surfaces, not generated snapshot collections. The request
surface may remain empty when no request is active; the experiment surface may
be absent until its first real experiment. Neither should be populated merely
...
Use `python -m scripts.workspace.workspace_cli reports status` for a read-only freshness check.
...
python -m scripts.workspace.workspace_cli reports refresh workspace
```
...
Overwrite current-state reports under `reports/current/`:

- `current/workspace_setup_report.md`
- `current/workspace_health_report.md`
- `current/manifest_validation_report.md`
- `current/protocol_validation_report.md`

Keep historical governance reports flat under `reports/history/` unless superseded intentionally:

- `history/git_governance_report.md`
- `history/report_drift_governance_report.md`
- `history/migration_dry_run_report.md`
- `history/manifest_portability_report.md`
