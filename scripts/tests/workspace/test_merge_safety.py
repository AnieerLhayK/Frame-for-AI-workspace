...
from scripts.workspace.merge_safety import assess, governed_assess

class MergeSafetyTests(unittest.TestCase):
    def test_disjoint_changes_are_safe(self) -> None:
        with patch("scripts.workspace.merge_safety.git", side_effect=["base", "M\ta.py", "A\tb.py"]):
            self.assertEqual(assess("", "left", "right")["status"], "SAFE_TO_CONTINUE")
    def test_structured_overlap_stops(self) -> None:
        with patch("scripts.workspace.merge_safety.git", side_effect=["base", "M\tconfig.yaml", "M\tconfig.yaml"]):
    ...
    def test_governed_ff_only_requires_review_evidence(self) -> None:
        ...
        with (
            patch("scripts.workspace.merge_safety.load_yaml", return_value=policy),
            patch("scripts.workspace.merge_safety.load_registry", return_value={"agents": {"codex": {"git_branch": "codex"}}}),
            patch("scripts.workspace.merge_safety.assess", return_value=base),
            patch("scripts.workspace.merge_safety.git", return_value=""),
            patch("scripts.workspace.merge_safety.git_returncode", return_value=0),
            patch("scripts.workspace.merge_safety.merge_review_note", return_value={"status": "completed"}),
        ):
    ...
    def test_governed_agent_integration_requires_task_record(self) -> None:
        ...
        with (
            patch("scripts.workspace.merge_safety.load_yaml", return_value=policy),
            patch("scripts.workspace.merge_safety.load_registry", return_value={"agents": {"codex": {"git_branch": "codex"}}}),
            patch("scripts.workspace.merge_safety.assess", return_value={"status": "SAFE_TO_CONTINUE", "findings": []}),
            patch("scripts.workspace.merge_safety.git", return_value=""),
            patch("scripts.workspace.merge_safety.git_returncode", return_value=0),
        ):
